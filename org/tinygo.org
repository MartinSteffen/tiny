...
***** TODO Analyze the Visitor_Stmt <2015-07-17 Fri 13:16>

#+begin_example

type Visitor_interface {

	Accept (stmt Stmt) (w Visitor)   // also called Visit 
}
     
#+end_example

     It's taken from walk.go.  What's strange he is also how it is later
     used, namely in the following form (at the beginning of the Walk, function): 
...
required there is to implement the accept method. So it's the combination
of both the data type plus the Visit-method that makes it to a visitor.
...
the return type are as requested by the Visitor interface.
