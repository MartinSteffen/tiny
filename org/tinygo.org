...
different, in particular the data structure being visited (see the CarPart
below) supports an accept function (which is ``standard'').

Let's start with the visitor interface.
...
The correspondent type in the ast is the =Visitor= interface. There are a
number of differences, though. The first one is more superficial, namely
that the =CarPartVisitor= interface requires /two/ visit-functions.  The
intention is (indicated also by its name) that the =CarPartVisitor= is
intended to visit car parts. However, car parts is only an interface, as
well, i.e., it's abstract (see the next code snippet). Therefore, what is
/concretely/ to be visited are instances of the /``sub-classes''/ of
=CarPart=. Of course 

not central. But it should be noted, that the visitor, despite its name, is
not supposed to directly visit elements of the type CarPart, at least not
/directly/. Let's first identify the car-parts. Being an inteface, there
are no ``direct'' instances of =CarPart=.
