...
******* TODO can one ``reuse constructors'' <2015-08-08 Sat 15:58>
	actually, it seems yes, but that's strange. The compiler does not give an error.
	I would have betted it's impossible. The following example, the two types are ok,
	but it seems that the latest one wins. I.e., the value is wrong because of the type.
