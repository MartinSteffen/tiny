...
it's the combination of both the data type plus the visit-method that makes
it to the visitor infrastructure.
...
supports the ~Visit~ function. Also the receiver type, the argument type
and the return type are as requested by the Visitor interface. This
...
because it's used in a special kind of assignment. In this case, not panic
occurs.
