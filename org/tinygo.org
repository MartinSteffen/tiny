...
*** DONE what's the overview of the stuff
    CLOSED: [2018-09-19 Wed 08:10]
    :PROPERTIES:
    :ID:       567a84d8-a287-4721-9f23-57e366f54747
    :END:
    :LOGBOOK:
    - State "DONE"       from "TODO"       [2018-09-19 Wed 08:10]
    :END:
<2017-07-19 Mi 16:58>
...
*** DONE Check the Pizza visitors <2015-07-21 Tue 16:23>
    CLOSED: [2018-08-28 Tue 20:39]
    :PROPERTIES:
    :ID:       cb8e5a9a-0833-4b80-b6fa-dd8573e97137
    :END:
    :LOGBOOK:
    - State "DONE"       from "TODO"       [2018-08-28 Tue 20:39]
    :END:
...
****** TODO why not make Visitor a super-interface? <2015-07-18 Sat 16:10>
       :PROPERTIES:
...
** QUESTION how to do a relocatable code and workspaces?
   :PROPERTIES:
   :ID:       e6c6c7c1-42e7-4f89-87fb-ec1e5444d04a
   :END:
<2017-07-19 Mi 20:11>

It's a bit unclear, how one can achieve that (and how other achieve
that). One thing of course is that one can have multiple entries.

The problem is the way that the go-documentation describes the
[[https://golang.org/doc/code.html#Workspaces][workspaces]]. It contains three subdirectories, ~src~, ~bin~, and ~pgk~.  The
default seems to be ~$(HOME)/go~ and that seems to indicate that one
intention is to have all the stuff inside there. It's strange to assume
that all clients use the same setup. If one checks it out /inside/ there,
then it's /inside/ the ~src~ dir, and that means one should not have
another ~src~-dir further down, it seems.



See also the [[https://golang.org/cmd/go/#hdr-GOPATH_environment_variable][link]] about the gopath. The go-path indeed is intended to be a
list. All of the 


It seems that it's the case that especially for github (as one example),
it's /remote import path/ (see again [[https://golang.org/cmd/go/#hdr-GOPATH_environment_variable][the documentation]])


** ANSWERED Why ~GOBIN~ 
   CLOSED: [2019-09-27 Fri 14:56]
   :PROPERTIES:
   :ID:       c0c2693a-f8c2-4951-831a-dfc02f7da63c
   :END:
   :LOGBOOK:
   - CLOSING NOTE [2019-09-27 Fri 14:56]
   :END:
...
If you want per-project bin/ directory then you can simply append your
project path to GOPATH, however you must have your code under
$project-path/src/ and go install will put all the binaries in
$project-path/bin.


****** DONE how does that embedding with register works <2015-08-10 Mon 12:01>
       CLOSED: [2019-09-27 Fri 14:56]
       :PROPERTIES:
       :ID:       6d91e3e7-067b-457b-9319-25405a5af61a
       :END:
       :LOGBOOK:
       - State "DONE"       from "TODO"       [2019-09-27 Fri 14:56]
       :END:
****** DONE check the various types and their access functions <2015-08-08 Sat 14:47>
       ...
       :END:
In my case it fails, but it's unclear why. Maybe look at ~go help gopath~



** QUESTION multiple GODIRS and bins                              
   :PROPERTIES:
   :ID:       2d9bbcfb-7480-4fa8-a25d-ebcbebb903c8
   :END:
<2017-07-20 Do 09:06>

Where does the bins show up in that case.



Let's best do an experiment.

Let's start with one 
#+BEGIN_SRC 
GOROOT=
GOPATH=/home/msteffen/go
#+END_SRC




** How should we arrange the stuff
...
** DONE why does go get does not work?
   CLOSED: [2017-07-20 Do 12:11]
   :LOGBOOK:
   - State "DONE"       from "BUG"        [2017-07-20 Do 12:11]
   :END:
<2017-07-20 Do 11:14>

no buildable Go source files in /home/msteffen/go/src/github.com/MartinSteffen/tiny

it seems that one  cannot get the whole ``repos'' but packages.


I try now [[../test2]].

In the first attempts it did not work.  The reason was I assume is that
under the expected path (in the GOPATH)
~/go/src/github.com/MartinSteffen/tiny, there was already some stuff.

After I removed it (move aside), then it worked (at least there is a
official git-clone again. If I do ~go get ast~ for example, I get a full
clone, not just the directories or packages I need. I guess git would not
support that anyway.
...
** DONE how was it done in the ~gotcha~ tool?
   CLOSED: [2017-07-20 Do 13:16]
   :LOGBOOK:
   - State "DONE"       from "TODO"       [2017-07-20 Do 13:16]
   :END:
<2017-07-20 Do 12:59>



The tool recommends ~go get github.com/akwick/gotcha~

und auch import mittels
...
** ANSWERED What's ~go get~
   CLOSED: [2018-05-13 Sun 16:17]
   :PROPERTIES:
   :ID:       b6e119c8-4697-47b6-b6df-fa2b2a7da86d
   :END:
   :LOGBOOK:
   - CLOSING NOTE [2018-05-13 Sun 16:17]
   :END:
<2017-07-20 Do 10:59>

See the [[https://golang.org/cmd/go/][documentation]]. 
...
** QUESTION Remote import paths 
   :PROPERTIES:
   :ID:       8fb260ed-f2ad-4f2c-b7c4-20eeed2bcda1
   :END:
<2017-07-20 Do 10:40>


How does the remote path work? Is it a convention? Are they supposed to be
taken from the net? See perhaps this [[http://engineeredweb.com/blog/2016/go-exposed-remote-import-paths/][blog]], but it's not clear there
either. Here's a [[https://medium.com/@c9s/golang-the-annoying-remote-import-path-c6c7e76517e5][critical opinion]].

It seems really that it's (when 





** DONE what's a go workspace 
   CLOSED: [2018-09-19 Wed 08:24]
   :PROPERTIES:
   :ID:       84343e5d-36c6-43b0-9fe0-88071793860e
   :END:
   :LOGBOOK:
   - State "DONE"       from "TODO"       [2018-09-19 Wed 08:24]
   :END:
<2017-07-19 Mi 16:59>
...
** TODO Make it portable, perhaps with remote import paths
   :PROPERTIES:
   :ID:       0a23c235-3582-40f6-9444-c99e83441ffa
   :END:
<2017-07-20 Do 09:51>


It's all fine, but it seems like a convention


** DONE GOPATH multiple directories?
   CLOSED: [2017-07-20 Do 06:27]
   :LOGBOOK:
   - State "DONE"       from "TODO"       [2017-07-20 Do 06:27]
   :END:
<2017-07-19 Mi 17:38>

Koennen there be more than one directory?
...
*** TODO What's actually the type of the Walk-function <2015-07-22 Wed 15:42>
    :PROPERTIES:
...
*** DONE what are anonymous fields <2015-08-13 Thu 11:38>
    CLOSED: [2018-09-19 Wed 08:34]
    :PROPERTIES:
    :ID:       e870ffff-48d2-4691-9ff3-5ff3202a02af
    :END:
    :LOGBOOK:
    - State "DONE"       from "TODO"       [2018-09-19 Wed 08:34]
    :END:
