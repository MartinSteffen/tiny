...
~CarPart~. Of course, there's no subclassing here. What matters is elements
that match the ~CarPart~ interface:
...
/type assertions/, like ~y := cp.(type)~). In a way it would be a
``hidden'' way of achieving the same --dispatch on the type argument-- as
with method overloading (which we don't have in go). 


**** TODO Why is there no ~visit~-function for cars? <2015-07-17 Fri 19:57>
